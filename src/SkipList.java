...
// }
    ...
    // ----------------------------------------------------------
    /**
     * Adjusts the head node when the new node's level is deeper than the
     * current list level.
     * 
     * @param newLevel
     *            The new maximum level of the list.
     */
    private void adjustHead(int newLevel) {
        ...
        // The new levels must be initialized to null, which the SkipNode
        // constructor does.
     ...
     * 
     * @param key
     *            The comparable key for the element.
     * @param elem
    ...
    public void insert(K key, Object elem) {
        ...
        // update[i] will store the node whose forward[i] pointer needs to be
        // updated.
        ...
        for (int i = level; i >= 0; i--) {
            // Traverse forward until the next node is null or its key is
            // greater than the key to insert
     ...
     * 
     * @param key
     *            The comparable key to search for.
    ...
    public Object find(K key) {
        ...
        // Iterate from the top level down to level 0
        for (int i = level; i >= 0; i--) {
            // Traverse forward until the next node is null or its key is >= the
            // search key
            while ((x.getForward()[i] != null) && (x.getForward()[i].key()
                .compareTo(key) < 0)) {
    ...
    public Object delete(K key) {
        ...
        for (int i = level; i >= 0; i--) {
            ...
            update[i] = x; // Store the node just before the potential deletion
                           // node
        }
        ...
        if (x != null && x.key().compareTo(key) == 0) {
            ...
            for (int i = 0; i <= level; i++) {
                // Check if the update node at level i points to the node to be
                // deleted (x)
                if (update[i].getForward()[i] == x) {
                    // Rewire: update[i] bypasses x and points to what x points
                    // to
                    update[i].getForward()[i] = x.getForward()[i];
                }
            }

            // 3. Adjust max level (level) if the deleted node was the highest
            // node
    ...
    private String getSortedElements() {
        ...
        SkipNode<K> curr = head.getForward()[0]; // Start at the first actual
                                                 // node (Level 0)
        ...
        while (curr != null) {
            ...
            // the sample concatenation
            sb.append(" Value (");

            sb.append(curr.toString()); // Calls KVPair.toString() ->
                                        // AirObject.toString()
            sb.append(")\r\n");
